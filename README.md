...
- **`iv_testjfe` null distribution aligns with `chi^2_{K-2}`.** At `K=20`, `N=3000` over 200 replications: empirical mean 17.7 vs target 18.0, variance 28.8 vs target 36.0, 95th percentile 27.5 vs target 28.9, empirical size 1.5% vs nominal 5%. Using structural residuals `u_i = y_i - hat_alpha - hat_beta * D_i` to estimate the pooled variance removes a first-stage binomial-variance component and makes the test mildly conservative in finite samples. A `method = "bootstrap"` option is available for small-`K` robustness.
